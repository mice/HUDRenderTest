...
- `Flush(string deviceTag, context?)` 在 Editor/测试环境优先落盘到项目内临时目录 `Logs/PerfProbe/perf_<tag>_<utc>.csv`（Player 环境回退到 `persistentDataPath`），文件内包含指标段 (`metric,avg,max`)；调用方可追加上下文段 (`context,value`)，用于记录 `useSlim` 等关键参数
