...
3. **执行命令**（来自 [.github/copilot-instructions.md](../../.github/copilot-instructions.md)）：
   ```powershell
   "C:\Program Files\Unity\Hub\Editor\2022.3.53f1c1\Editor\Unity.exe" -batchmode -quit -nographics `
     -projectPath "D:\ProjCommon\HUDRenderTest" -runTests -testPlatform EditMode `
     -testResults "Logs\EditModeTests.xml"
   ```
...
- **依赖**：无
- **涉及文件**：
  - `Assets/Scripts/UIDataRender/UIMeshData.cs`（删除 `using TreeEditor`）
  - `Assets/Scripts/UIDataRender/UIMeshDataX.cs`（删除 `using TreeEditor`）
  - `Assets/Scripts/UIDataRender/UIPrefabManager.cs`（删除 `using UnityEditor`，或用 `#if UNITY_EDITOR` 包裹）
...
- **涉及文件**：
  ...
  - 修改 `Assets/Scripts/UIDataRender/UIGeometry.cs`（内部委托）
...
- **涉及文件**：
  ...
  - 修改 `Assets/Scripts/UIDataRender/UIGeometry.cs`（使用 `GeometryBuffer`）
...
- **依赖**：TASK-04
- **涉及文件**：`Assets/Scripts/UIDataRender/UIMeshDataX.cs`
...
- **依赖**：TASK-06
- **涉及文件**：`Assets/Scripts/UIDataRender/UIMeshDataX.cs`、`Assets/Scripts/UIDataRender/Core.cs`（接口添加 `FillWithMatrix`）、`Assets/Scripts/UIDataRender/UIMeshData.cs`（声明已实现）
...
- **改动**：`MaxImageSlots` 默认 3；`Register` 超容量返回 `-1` 并告警，此返回值只表示当前 slot table 容量不足，最终由 `MergeBatcher` 拆分到新的 Draw Call。
...
- **依赖**：TASK-11
- **涉及文件**：新增 `Assets/Scripts/UIDataRender/Render/MergeBatcher.cs`、`RenderBatch.cs`、`MaterialBinder.cs`
- **改动**：在合并循环外增加分批阶段；超阈值时 `LogWarning`
- **验收测试 ID**：TC-MB-01..03
- **风险**：中
...
- **依赖**：TASK-11
- **涉及文件**：`Assets/Scripts/UIDataRender/UIPrefabRegistration.cs`、`DataPrefabHolder.cs`
...
- **改动**：插入到 `UIPrefabHolder.Fill` / Job 调度 / `Graphics.DrawMesh` 三个埋点
- **验收测试 ID**：TC-PP-01、TC-PP-02
- **风险**：低
...
### TASK-19：Burst SoA 改造（可选）

- **依赖**：TASK-18 数据支持
- **涉及文件**：新增 `Assets/Scripts/UIDataRender/Jobs/MergeJobs.Burst.cs`
- **改动**：将 `UIMeshData[]` 平铺为 `NativeArray<MeshSlimSoA>` + `NativeArray<Vector3>` 等，启用 `[BurstCompile]`
- **验收**：与 `ManagedCodeInJob` 路径输出等价 + 性能 ≥ 2× 提升
